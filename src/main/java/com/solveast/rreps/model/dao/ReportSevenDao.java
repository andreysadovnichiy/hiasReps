...
public class ReportSevenDao {
    ...
    public List<Query7> getQuery() {
        ...
        String sql = "SELECT " +
                " cl.client_id, ls.court_case_id AS court_case_id, ls.court_case_lawsuit_id, ccsn.court_case_state_id, ccsn.name, ls.ladget_date, ls.decision_date, cs.ms_rejection_cd, ls.instance_cd, ls.court_decision_cd," +
                ...
                " WHERE (ccsn.language_cd = 'en') AND (pc.language_cd = 'en') AND" +
                " (rs.file_status_id > 0 OR rs.file_status_id IS NULL)";
        ...
        List<Query7> query =
                (List<Query7>) namedParameterJdbcTemplate.query(sql, namedParameters, new RowMapper<Query7>() {
                    ...
                    public Query7 mapRow(ResultSet rs, int i) throws SQLException {
                        ...
                        item.setCourtCaseId(rs.getLong("court_case_id"));
                        item.setCourtCaseLawsuitId(rs.getLong("court_case_lawsuit_id"));
